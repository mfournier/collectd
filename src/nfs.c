...
char *nfs_event_counters[] = {"inoderevalidates",
                              "dentryrevalidates",
                              "datainvalidates",
                              "attrinvalidates",
                              "vfsopen",
                              ...
                              "shortwrites",
                              "delay"};
#define NB_NFS_EVENT_COUNTERS                                                  \
  (sizeof(nfs_event_counters) / sizeof(*nfs_event_counters))

char *nfs_byte_counters[] = {"normalreadbytes", "normalwritebytes",
                             "directreadbytes", "directwritebytes",
                             "serverreadbytes", "serverwritebytes",
                             "readpages",       "writepages"};
#define NB_NFS_BYTE_COUNTERS                                                   \
  (sizeof(nfs_byte_counters) / sizeof(*nfs_byte_counters))

/* See /net/sunrpc/xprtsock.c in Linux Kernel sources */
char *nfs_xprt_udp[] = {"port",    "bind_count",    "rpcsends",   "rpcreceives",
                        "badxids", "inflightsends", "backlogutil"};
...
char *nfs_xprt_rdma[] = {
    "port",           "bind_count",     "connect_count", "connect_time",
    "idle_time",      "rpcsends",       "rpcreceives",   "badxids",
    "backlogutil",    "read_chunks",    "write_chunks",  "reply_chunks",
    "total_rdma_req", "total_rdma_rep", "pullup",        "fixup",
    "hardway",        "failed_marshal", "bad_reply"};
#define NB_NFS_XPRT_RDMA (sizeof(nfs_xprt_rdma) / sizeof(*nfs_xprt_rdma))

#define MAX3(x, y, z)                                                          \
  (((((x) > (y)) ? (x) : (y)) > (z)) ? (((x) > (y)) ? (x) : (y)) : (z))
#define NB_NFS_XPRT_ANY                                                        \
  (MAX3(NB_NFS_XPRT_UDP, NB_NFS_XPRT_TCP, NB_NFS_XPRT_RDMA))
...
static int config_nfs_mountpoint_add(oconfig_item_t *ci) /* {{{ */
{
  ...
  if (0 == c_avl_get(config_mountpoints, ci->values[0].value.string, &dummy)) {
    WARNING(
        "nfs plugin: 'Mountpoint' %s defined twice (ignoring this occurrence)",
...
static int is_proc_self_mountstats_available(void) /* {{{ */
{
  ...
  if (NULL == fh) {
    ...
    if (uname(&uname_data)) {
      WARNING("nfs plugin : Could not open /proc/self/mountstats. And Linux "
              "kernel info (from uname) is unavailable");
    } else {
      char *str, *end;
      int k_version[3];
      ...
      for (i = 0; i < 3; i++) {
        ...
        if ((k_version[0] >= 3))
          break; /* Supported since 2.6.27 so no need to continue */
        if (end[0] == '.')
          end++;
        str = end;
      }
      if (parse_ok) {
        if (k_version[0] >= 3)
          print_warning = 1; /* Kernel >= 3.x   */
        else if (k_version[0] < 2)
          print_warning = 0; /* Kernel < 2.x    */
        else {               /* kernel 2.x */
          if (k_version[1] < 6)
            print_warning = 0; /* Kernel < 2.6    */
          else {               /* 2.6.x (or upper !?) */
            if (k_version[2] < 17)
              print_warning = 0; /* Kernel < 2.6.17  */
            else
              print_warning = 1; /* Kernel >= 2.6.17 */
          }
        }
        if (print_warning) {
          WARNING("nfs plugin : Could not open /proc/self/mountstats. You have "
                  "kernel %s and this is supported since 2.6.17",
                  kv);
        }
      } else {
        WARNING("nfs plugin : Could not open /proc/self/mountstats. And kernel "
                "version could not be parsed (%s)",
                kv);
      }
      free(kv);
    }
    INFO("nfs plugin : Could not open /proc/self/mountstats. This is normal if "
...
#define NFSPLUGININFO "nfs plugin DEBUG "
  ...
  for (i = 0; i < NB_NFS_EVENT_COUNTERS; i++) {
    INFO(NFSPLUGININFO "event (%20s) : '%Lu'", nfs_event_counters[i],
         m->events[i]);
  }
  for (i = 0; i < NB_NFS_BYTE_COUNTERS; i++) {
    INFO(NFSPLUGININFO "bytes (%20s) : '%Lu'", nfs_byte_counters[i],
         m->bytes[i]);
  }
  switch (m->xprt_type) {
  ...
  for (i = 0; i < m->nb_op; i++) {
    INFO(NFSPLUGININFO "Per op (%20s) : %Lu %Lu %Lu %Lu   %Lu %Lu %Lu %Lu",
         m->op[i].op_name, m->op[i].op[0], m->op[i].op[1], m->op[i].op[2],
         m->op[i].op[3], m->op[i].op[4], m->op[i].op[5], m->op[i].op[6],
         m->op[i].op[7]);
  }
                                             ...
                                             const char *data_type) /* {{{ */
{
  ...
  sstrncpy(vl->plugin_instance, m->mountpoint, sizeof(vl->plugin_instance));
  sstrncpy(vl->type, data_type, sizeof(vl->type));
  for (i = 0; vl->plugin_instance[i]; i++) {
    if (!(((vl->plugin_instance[i] >= 'A') &&
           (vl->plugin_instance[i] <= 'Z')) ||
          ((vl->plugin_instance[i] >= 'a') &&
           (vl->plugin_instance[i] <= 'z')) ||
          ((vl->plugin_instance[i] >= '0') &&
           (vl->plugin_instance[i] <= '9')))) {
...
static void mountstats_compute_and_submit(mountstats_t *m,
                                          mountstats_t *oldm) /* {{{ */
{
#define NFS_OPERATIONS_NB 2
  ...
  size_t i;
  value_t values[MAX3(NB_NFS_BYTE_COUNTERS, NB_NFS_EVENT_COUNTERS,
                      NB_NFS_XPRT_ANY)];
  unsigned long long rpcsends, backlogutil,
      ops_since_last_time[NFS_OPERATIONS_NB];
  derive_t sends, backlog, ops[NFS_OPERATIONS_NB], kilobytes[NFS_OPERATIONS_NB];
  gauge_t retrans[NFS_OPERATIONS_NB], kb_per_op[NFS_OPERATIONS_NB],
      rtt_per_op[NFS_OPERATIONS_NB], exe_per_op[NFS_OPERATIONS_NB];
  ...
  for (i = 0; i < NFS_OPERATIONS_NB; i++) {
    ...
    for (j = 0; j < m->nb_op; j++) {
      if (((i == 0) && (!strcmp("READ", m->op[j].op_name))) ||
    ...
    for (j = 0; j < oldm->nb_op; j++) {
      if (((i == 0) && (!strcmp("READ", oldm->op[j].op_name))) ||
    ...
    /* gauge   */ retrans[i] = (m->op[i1].op[1] - oldm->op[i2].op[1]) -
                               (m->op[i1].op[0] - oldm->op[i2].op[0]);
    /* derive  */ kilobytes[i] = ((m->op[i1].op[3] - oldm->op[i2].op[3]) +
                                  (m->op[i1].op[4] - oldm->op[i2].op[4])) /
                                 1024;
    if (ops_since_last_time[i]) {
      /* gauge   */ kb_per_op[i] = kilobytes[i] / ops_since_last_time[i];
      /* gauge   */ rtt_per_op[i] =
          (m->op[i1].op[6] - oldm->op[i2].op[6]) / ops_since_last_time[i];
      /* gauge   */ exe_per_op[i] =
          (m->op[i1].op[7] - oldm->op[i2].op[7]) / ops_since_last_time[i];
    } else {
      /* gauge   */ kb_per_op[i] = 0;
  ...
  vl.values_len = 2;
  values[0].gauge =
      ops_since_last_time[0] ? (retrans[0] * 100 / ops_since_last_time[0]) : 0;
  values[1].gauge =
      ops_since_last_time[1] ? (retrans[1] * 100 / ops_since_last_time[1]) : 0;
  plugin_dispatch_values(&vl);
...
static int duplicate_mountstats(mountstats_t *dest, mountstats_t *src) {
  ...
  for (i = 0; i < src->nb_op; i++) {
    if ((!strcmp(src->op[i].op_name, "READ")) ||
        (!strcmp(src->op[i].op_name, "WRITE"))) { /* Copy only needed ops */
      strncpy(dest->op[dest->nb_op].op_name, src->op[i].op_name,
              sizeof(src->op[i].op_name));
...
static int remove_old_mountpoints(time_t t) /* {{{ */
{
   ...
   *   When a mountpoint does not have its last_updated equal to t, put it in
   * the **remove list.
  ...
  for (i = 0; i < n; i++) {
    INFO("nfs plugin : mountpoint '%s' no more monitored (not found in "
         "/proc/self/mountstats)",
         key);
    if (0 == c_avl_remove(mountstats_per_mountpoint, remove[i], (void *)&key,
                          (void *)&m)) {
      free(key);
      free_mountstats(m);
    }
...
static int dispatch_mountstats(mountstats_t *m) /* {{{ */
{
  ...
  if (0 == c_avl_get(mountstats_per_mountpoint, m->mountpoint, (void *)&oldm)) {
    ...
    INFO("nfs plugin : mountpoint '%s' is now monitored (found in "
         "/proc/self/mountstats)",
         key);
  }
  assert(oldm != NULL);
  /* Keep a copy of the new mountstats into oldm for next time */
  if (0 != duplicate_mountstats(oldm, m)) {
    return (-1);
  }
  return (0);
} /* }}} dispatch_mountstats */

static int string_to_array_of_Lu(char *str, unsigned long long *a,
                                 int n) /* {{{ */
{
  char *s, *endptr;
  int i;
...
static int parse_proc_self_mountstats(void) /* {{{ */
{
  ...
  fh = fopen("/proc/self/mountstats", "r");
  if (NULL == fh) {
    WARNING("nfs plugin : Could not open /proc/self/mountstats. But it could "
            "be opened at plugin initialization. Strange...");
    return (-1);
  }
  now = time(NULL);
  ...
  while (fgets(buf, sizeof(buf), fh)) {
    ...
    case psm_state_start: /* Line is starting with "device" (or should be) */
      ...
      while ((((str[0] == ' ') || str[0] == '\t')) && (str > nfsdir))
        str--; /* remove extra spaces */
      str[1] = '\0';
      mountstats.mountpoint = strdup(
          nfsdir); /* Keep a copy as nfsdir was a pointer to the buffer */
#endif
    ...
    case psm_state_device_nfs:
      ...
      if (!strncmp(str, "age:", sizeof("age:") - 1)) {
        ...
        int n = string_to_array_of_Lu(str + sizeof("events:"),
                                      mountstats.events, NB_NFS_EVENT_COUNTERS);
        if (n != NB_NFS_EVENT_COUNTERS) {
          goto an_error_happened;
        }
      } else if (!strncmp(str, "bytes:", sizeof("bytes:") - 1)) {
        int n = string_to_array_of_Lu(str + sizeof("bytes:"), mountstats.bytes,
                                      NB_NFS_BYTE_COUNTERS);
        ...
        if (!strncmp(str, "tcp ", sizeof("tcp ") - 1)) {
          n = string_to_array_of_Lu(str + sizeof("tcp ") - 1, mountstats.xprt,
                                    NB_NFS_XPRT_TCP);
          n -= NB_NFS_XPRT_TCP;
          mountstats.xprt_type = nfs_xprt_type_tcp;
        } else if (!strncmp(str, "udp ", sizeof("udp ") - 1)) {
          n = string_to_array_of_Lu(str + sizeof("udp ") - 1, mountstats.xprt,
                                    NB_NFS_XPRT_UDP);
          n -= NB_NFS_XPRT_UDP;
          mountstats.xprt_type = nfs_xprt_type_udp;
        } else if (!strncmp(str, "rdma ", sizeof("rdma ") - 1)) {
          n = string_to_array_of_Lu(str + sizeof("rdma ") - 1, mountstats.xprt,
                                    NB_NFS_XPRT_RDMA);
          n -= NB_NFS_XPRT_RDMA;
      ...
      } else if (!strncmp(str, "per-op statistics",
                          sizeof("per-op statistics") - 1)) {
    ...
    case psm_state_device_nfs_per_opt_stats:
      ...
      if (mountstats.nb_op >= mountstats.size_op) {
        if (NULL == (mountstats.op = realloc(mountstats.op,
                                             (mountstats.size_op + 50) *
                                                 sizeof(*mountstats.op)))) {
          ERROR("nfs plugin : out of memory");
          clear_mountstats(&mountstats);
          fclose(fh);
          return (-1);
      ...
      sstrncpy(mountstats.op[mountstats.nb_op].op_name, str,
               ((i + 1) > sizeof(mountstats.op[mountstats.nb_op].op_name))
                   ? sizeof(mountstats.op[mountstats.nb_op].op_name)
                   : (i + 1));
      str += i + 1;
      if (8 !=
          string_to_array_of_Lu(str, mountstats.op[mountstats.nb_op].op, 8)) {
        goto an_error_happened;
      }
      mountstats.nb_op++;
      break;
    default:
      ERROR("nfs plugin : unknown state (bug) while parsing "
            "'/proc/self/mountstats' (buffer was '%s')",
            buf);
      assert(3 == 4);
    }
  }
  if (feof(fh)) {
    ...
    WARNING("nfs plugin : Reading /proc/self/mountstats failed. Some data will "
            "be ignored.");
    fclose(fh);
    return (-1);
  }
...
an_error_happened:
  fclose(fh);
  clear_mountstats(&mountstats);
  ERROR("nfs plugin : parse error while reading /proc/self/mountstats (state "
        "was %d, buffer was '%s')",
        state, buf);
  return (-1);
...
static int nfs_init(void) {
  proc_self_mountstats_is_available =
      (0 == is_proc_self_mountstats_available()) ? 1 : 0;
  INFO("nfs plugin : Statistics through /proc/self/mountstats are %s",
       proc_self_mountstats_is_available ? "available" : "unavailable");
...
static int nfs_read(void) {
  ...
  if (proc_self_mountstats_is_available && config_mountpoints)
    parse_proc_self_mountstats();
  return 0;
}
/* #endif KERNEL_LINUX */
